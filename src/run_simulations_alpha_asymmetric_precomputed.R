#!/usr/bin/env Rscript
# OPTIMIZED VERSION: Parallel execution with pre-computed term2 for massive speedup
# Pre-computes term2 once per alpha/method/n_counts combination (10,000 samples)
# Dramatically reduces computation time by avoiding redundant distance matrix calculations
...
# Usage: Rscript run_simulations_alpha_asymmetric_precomputed.R
...
cat("=========================================\n")
cat("OPTIMIZED PARALLEL SIMULATION RUNNER\n")
cat("WITH PRE-COMPUTED TERM2\n")
cat("=========================================\n\n")
...
# ==============================================================================
# OPTIMIZATION: Pre-computation functions
# ==============================================================================

# Function to pre-compute term2 for a given alpha and n_counts
precompute_term2 <- function(alpha, n_counts, n_samp = 10000, method = "mn", seed = 999) {
  set.seed(seed)
  K <- length(alpha)
  ...
  # Compute term2 once
  cat("    Pre-computing term2 for", method, "method (n_samp =", n_samp, ")...\n")
...
# ==============================================================================
# OPTIMIZED es_sampling function with pre-computed term2
# ==============================================================================
es_sampling <- function(alpha = c(2,4,4),
                           ...
                           nsim = 1000,
                           seed = 42,
                           term2_correct_precomputed = NULL,
                           term2_incorrect_precomputed = NULL,
                           parallel = TRUE,
  ...
  if(parallel && n_cores > 1) {
    results <- parallel::mclapply(1:nsim, function(i) {
      ...
      # OPTIMIZATION: Calculate only term1 (term2 is pre-computed)
      term1_correct <- calculate_term1_only(y = as.numeric(p), dat = p_matrix)
      term1_incorrect <- calculate_term1_only(y = as.numeric(p), dat = p_matrix_wrong)
      term1_correct_vs_true_mean <- calculate_term1_only(y = true_mean, dat = p_matrix)
      term1_incorrect_vs_true_mean <- calculate_term1_only(y = true_mean, dat = p_matrix_wrong)

      # Use pre-computed term2
      es_correct <- term1_correct - term2_correct_precomputed
      es_incorrect <- term1_incorrect - term2_incorrect_precomputed
      es_correct_vs_true_mean <- term1_correct_vs_true_mean - term2_correct_precomputed
      es_incorrect_vs_true_mean <- term1_incorrect_vs_true_mean - term2_incorrect_precomputed

      # For return, use pre-computed term2 values
      term2_correct <- term2_correct_precomputed
      term2_incorrect <- term2_incorrect_precomputed
      term2_correct_vs_true_mean <- term2_correct_precomputed
      term2_incorrect_vs_true_mean <- term2_incorrect_precomputed

      return(c(
        es_correct = es_correct,
        es_incorrect = es_incorrect,
        es_correct_vs_true_mean = es_correct_vs_true_mean,
        es_incorrect_vs_true_mean = es_incorrect_vs_true_mean,
        term1_correct = term1_correct,
        term1_incorrect = term1_incorrect,
        term1_correct_vs_true_mean = term1_correct_vs_true_mean,
        term1_incorrect_vs_true_mean = term1_incorrect_vs_true_mean,
        term2_correct = term2_correct,
        term2_incorrect = term2_incorrect,
    ...
    for(i in 1:nsim){
      ...
      # OPTIMIZATION: Calculate only term1 (term2 is pre-computed)
      term1_correct[i] <- calculate_term1_only(y = as.numeric(p), dat = p_matrix)
      term1_incorrect[i] <- calculate_term1_only(y = as.numeric(p), dat = p_matrix_wrong)
      term1_correct_vs_true_mean[i] <- calculate_term1_only(y = true_mean, dat = p_matrix)
      term1_incorrect_vs_true_mean[i] <- calculate_term1_only(y = true_mean, dat = p_matrix_wrong)

      # Use pre-computed term2
      es_correct[i] <- term1_correct[i] - term2_correct_precomputed
      es_incorrect[i] <- term1_incorrect[i] - term2_incorrect_precomputed
      es_correct_vs_true_mean[i] <- term1_correct_vs_true_mean[i] - term2_correct_precomputed
      es_incorrect_vs_true_mean[i] <- term1_incorrect_vs_true_mean[i] - term2_incorrect_precomputed

      # For return, use pre-computed term2 values
      term2_correct[i] <- term2_correct_precomputed
      term2_incorrect[i] <- term2_incorrect_precomputed
      term2_correct_vs_true_mean[i] <- term2_correct_precomputed
      term2_incorrect_vs_true_mean[i] <- term2_incorrect_precomputed
    }
  }
  return(list(
...
# ==============================================================================
# OPTIMIZED es_mn_sampling function with pre-computed term2
# ==============================================================================
...
# ==============================================================================
# OPTIMIZED run_simulation_experiment with term2 pre-computation
# ==============================================================================
run_simulation_experiment <- function(alpha, alpha_wrong, n_samp = 100,
  ...
  # OPTIMIZATION: Pre-compute term2 for all methods and n_counts combinations
  cat("  Pre-computing term2 values for all n_counts...\n")
  term2_cache <- list()
  for (n in n_counts_vec) {
    term2_cache[[as.character(n)]] <- list(
      correct_nonmn = precompute_term2(alpha, n, n_samp = 10000, method = "nonmn", seed = base_seed + 1000),
      incorrect_nonmn = precompute_term2(alpha_wrong, n, n_samp = 10000, method = "nonmn", seed = base_seed + 2000),
      correct_mn = precompute_term2(alpha, n, n_samp = 10000, method = "mn", seed = base_seed + 3000),
      incorrect_mn = precompute_term2(alpha_wrong, n, n_samp = 10000, method = "mn", seed = base_seed + 4000)
    )
  }
  ...
  for (i in seq_along(n_counts_vec)) {
    ...
    # Get pre-computed term2 values for this n_counts
    tc <- term2_cache[[as.character(n)]]

    # Run simulations with pre-computed term2
    result_nonmn <- es_sampling(alpha = alpha, alpha_wrong = alpha_wrong, n_samp = n_samp,
                                seed = base_seed + i, nsim = nsim, n_counts = n,
                                term2_correct_precomputed = tc$correct_nonmn,
                                term2_incorrect_precomputed = tc$incorrect_nonmn,
    ...
    result_mn <- es_mn_sampling(alpha = alpha, alpha_wrong = alpha_wrong, n_samp = n_samp,
                               ...
                               term2_correct_precomputed = tc$correct_mn,
                               term2_incorrect_precomputed = tc$incorrect_mn,
                               parallel = parallel, n_cores = n_cores)
...
# Define all scenarios with ASYMMETRIC true alpha
# True alpha = c(2, 3, 15): asymmetric, skewed toward third category, sum=20
scenarios <- list(
  list(
...
if (in_gui) {
  ...
  cat("  Rscript src/run_simulations_alpha_asymmetric_precomputed.R\n\n")
